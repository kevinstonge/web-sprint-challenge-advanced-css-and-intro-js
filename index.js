...
function getHTML(artists){
    ...
    artists.forEach(e => {
      html += 
        ...
        <div class="artist">
          <div class="image">
            <img src="assets/${e.name.replace(" ","-")}.jpg"/>
          </div>
          <div class = "name">
            <a href="https://en.wikipedia.org/wiki/${e.name.replace(" ","_")}">${e.name}</a>
          </div>
...
console.log(document.querySelector(".artists-container"));
document.querySelector(".artists-container").innerHTML = getHTML(artists);
//some of the images are missing, it would be easy to gather the missing ones and add them to the folder, but I decided not to
...
function randomize(artists){
    ...
    //just picks a random position to move each element to as it iterates through the entire array
    let i = 0;
    let j = 0;
